...
What it does (see header of `release.sh`): polished changelog → notarize → EdDSA-sign → appcast → `gh release create`. Tag is `v0.3.2` (unprefixed, GH Release auto-creates from `gh release create`). Last shipped: **0.10.3**.
...
What it does (see header of `testflight.sh`): polished changelog → archive → export IPA → `altool` upload → poll ASC for VALID → set "What to Test" via ASC API. Last shipped: **0.10.4 (263)** as `ios/v0.10.4`.
