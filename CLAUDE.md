...
What it does (see header of `release.sh`): polished changelog → notarize → EdDSA-sign → appcast → `gh release create`. Tag is `v0.3.2` (unprefixed, GH Release auto-creates from `gh release create`). Last shipped: **0.9.10**.
